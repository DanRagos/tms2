<?php
	...
	if(isset($_POST['icm_set'])) {	
		$mem_id=$_POST['mem_id'];
		$docu_id=$_POST['docu_id'];
		$receive_agen=$_POST['receive_agen'];
		$phase=$_POST['phase'];
		...
		$queryun= "select * from file WHERE name='$file_name'";
			...
			$row = $conn->query("SELECT * FROM `file` where name='$file_name'") or die(mysqli_error());
				...
				$sql2= "INSERT INTO `incoming_comm`(id, file_id, file_name, agency, purpose, phase, date) values (NULL, '$file_name1', '$file_name', '$receive_agen', '$purpose', '$phase', '$inc_date')";
