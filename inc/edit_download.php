<?php
	...
	mysqli_query($conn,"update memorandum_agreement set effective_date='$eff_date', parties_involve='$part_agency', description='$moa_description' where file_id='$file_id'");
		...
		$mim_location=$_POST['mim_location'];
		$participants=$_POST['participants'];
		$mim_description=$_POST['mim_description'];
	mysqli_query($conn,"update minutes_meeting set date='$date_meeting', location='$mim_location', participants = '$participants' ,description='$mim_description' where file_id='$file_id'");
	 if ($_SESSION['type']!="admin") {
        	echo "
			<script>alert('File $name_file edited')</script>
			<script>window.location = '../user/file_download.php'</script>
